...
.impact h4,.affected h4 { font-size: 13px; margin-bottom: 8px }
.impact p { font-size: 14px; color: var(--muted); overflow-wrap: anywhere }
...
th { font-weight: 600 }
thead th { padding: 12px; font-size: 11px; color: var(--muted); border-bottom: 1px solid var(--line); background: #f1f6f9; vertical-align: bottom }
td,tbody th { padding: 14px 12px; border-bottom: 1px solid var(--line); vertical-align: top; overflow-wrap: anywhere }
tbody tr:last-child>* { border-bottom: 0 }
.evidence-table { table-layout: fixed }
.evidence-table thead th:first-child { width: 36% }.evidence-table thead th:nth-child(2) { width: 27% }
.evidence-table code,.evidence-table pre { font-size: 12px }
.source-link { color: #006577; text-decoration: underline; text-decoration-color: #a6c6cd; text-underline-offset: 3px }
.affected { margin-top: 22px }.affected ul { list-style: none; margin: 0; padding: 0 }
.affected li { display: flex; gap: 20px; justify-content: space-between; font-size: 13px; padding: 12px 0; border-bottom: 1px solid var(--line) }
.affected li:last-child { border: 0 }
.affected .affected-context { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; font-size: 12px; color: var(--muted) }
...
.inventory-group[open]>.table-scroll { padding: 0 10px 10px }
.inventory-table { table-layout: fixed }.inventory-table thead th:first-child { width: 34% }.inventory-table thead th:nth-child(2) { width: 23% }
.inventory-table tbody th { font-weight: 400 }.inventory-table tbody th,.inventory-table tbody td { padding: 10px 12px }.inventory-table tbody tr:hover { background: #fafcfd }
.inventory-item-name { display: block; padding-top: 7px; line-height: 1.5; overflow-wrap: anywhere }.inventory-item-name strong,.inventory-item-name .observation-identity strong { font-size: 13px; font-weight: 600 }
.inventory-table .client-identity { padding-top: 7px }.inventory-context { display: block; font-size: 11px; color: var(--muted); margin-top: 4px; line-height: 1.6 }
.inventory-location { display: block; margin-top: 5px; color: var(--muted); font-size: 11px; font-weight: 400 }
.state { display: inline-block; font-size: 11px; color: var(--muted); margin-top: 4px }.state-enabled { color: #006577 }.state-disabled { padding: 2px 6px; background: #f0f2f5; border-radius: 5px }
.inventory-metadata>summary { display: flex; align-items: center; gap: 8px; min-height: 44px; width: 100%; padding: 7px 0; color: var(--brand); list-style: none }
.inventory-path { font-size: 11px; font-weight: 400; color: var(--muted); min-width: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap }.inventory-detail-label { font-size: 11px; flex: none }
.inventory-metadata[open] .inventory-path { white-space: normal; overflow-wrap: anywhere }.inventory-metadata>summary:hover .inventory-detail-label { text-decoration: underline }
.metadata-list { margin: 5px 0 8px; padding: 6px 12px; background: var(--surface); border: 1px solid var(--line); border-radius: 10px; font-size: 11px }
.metadata-list>div { padding: 8px 0; border-bottom: 1px solid var(--line) }.metadata-list>div:last-child { border: 0 }.metadata-list dt { color: var(--faint); font-size: 11px; margin-bottom: 3px }.metadata-list dd { margin: 0 }.metadata-list pre,.metadata-list code { font-size: 11px; white-space: pre-wrap; overflow-wrap: anywhere }

/* Coverage colors describe collection states, never an invented score. */
.discovery-summary { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: 18px; margin: 0 0 20px }
...
.coverage-panel { padding: 28px 30px 0; border: 1px solid var(--line); border-radius: var(--radius); background: var(--white); box-shadow: var(--shadow); overflow: hidden }
...
.coverage-chart { display: block; width: 100%; height: 10px; border-radius: 5px; overflow: hidden }
.coverage-collected { fill: #007a93; background: #007a93 }.coverage-missing { fill: #cbd5e1; background: #cbd5e1 }.coverage-skipped { fill: #d4b16e; background: #d4b16e }.coverage-error { fill: #b96750; background: #b96750 }.coverage-unknown { fill: #94a3b8; background: #94a3b8 }
.coverage-legend { display: flex; gap: 12px 26px; flex-wrap: wrap; margin: 20px 0 24px }.coverage-stat { display: flex; align-items: center; gap: 7px; color: var(--muted); font-size: 12px }.coverage-stat strong { color: var(--ink); font-variant-numeric: tabular-nums }.coverage-dot { width: 8px; height: 8px; border-radius: 3px }
.sources-disclosure { margin: 0 -30px; border-top: 1px solid var(--line); background: #fbfdfe }.sources-disclosure>summary { display: flex; align-items: center; gap: 12px; min-height: 54px; padding: 14px 30px; list-style: none; font-size: 13px; font-weight: 500 }.sources-disclosure>summary>span:nth-child(2) { margin-left: auto; color: var(--faint); font-size: 12px; font-weight: 400 }.sources-disclosure>summary:hover { background: #f0f8fa; color: var(--brand) }
.sources-table { table-layout: fixed }.sources-table thead th:first-child { width: 48% }.sources-table thead th:nth-child(2) { width: 17% }.sources-table tbody th { font-weight: 500 }.sources-table tbody tr:target { background: #eaf8fb }
.source-status { font-size: 12px; font-weight: 600 }.source-collected { color: var(--brand) }.source-error { color: var(--high) }.source-skipped { color: var(--medium) }.source-reason { display: block; font-size: 12px; color: var(--muted); margin-top: 5px }
...
@media(max-width:560px) {
  ...
  .finding { padding: 22px 18px 0 }.finding-topline { gap: 9px; font-size: 11px }.finding-clients { width: 100%; margin-left: 0 }.finding h3 { font-size: 20px }.finding-summary { font-size: 14px }.next-step { padding: 14px 12px; gap: 9px }.next-step p { font-size: 13px }.finding-evidence { margin: 0 -18px }.finding-evidence>summary { padding: 13px 18px; font-size: 11px; gap: 9px }.evidence-count { font-size: 11px; white-space: nowrap }.evidence-content { padding: 20px 18px }.evidence-meta { font-size: 11px; gap: 7px 12px }.affected li { flex-direction: column; gap: 7px }
  .evidence-table thead,.sources-table thead { position: absolute; width: 1px; height: 1px; overflow: hidden; clip-path: inset(50%) }.evidence-table,.evidence-table tbody,.sources-table,.sources-table tbody { display: block }.evidence-table tr,.sources-table tr { display: block; padding: 12px 0; border-bottom: 1px solid var(--line) }.evidence-table td,.sources-table td,.sources-table tbody th { display: block; border: 0; padding: 5px 8px }.evidence-table td:before,.sources-table td:before,.sources-table tbody th:before { display: block; margin-bottom: 4px; color: var(--faint); font: 11px Onest,system-ui,sans-serif }.evidence-table td:nth-child(1):before { content: "Source" }.evidence-table td:nth-child(2):before { content: "Setting / evidence" }.evidence-table td:nth-child(3):before { content: "Observed value" }.sources-table tbody th:before { content: "Source" }.sources-table td:nth-child(2):before { content: "Scope" }.sources-table td:nth-child(3):before { content: "Collection result" }
  ...
  .inventory-table,.inventory-table tbody { display: block }.inventory-table thead { position: absolute; width: 1px; height: 1px; overflow: hidden; clip-path: inset(50%) }.inventory-table .inventory-row { display: grid; grid-template-columns: minmax(0,1fr) minmax(90px,42%); border-bottom: 1px solid var(--line); padding: 8px 0 }.inventory-table .inventory-row:last-child { border-bottom: 0 }.inventory-table tbody th,.inventory-table tbody td { width: auto; border: 0; padding: 2px 5px }.inventory-table tbody td:last-child { grid-column: 1/-1 }.inventory-table .client-identity { font-size: 11px; gap: 5px }.inventory-table .client-identity .brand-icon { width: 15px; height: 15px }
  .discovery-summary { grid-template-columns: repeat(2,minmax(0,1fr)); gap: 12px }.discovery-summary>div { padding: 15px }.discovery-summary dd { font-size: 24px }.coverage-panel { padding: 24px 20px 0 }.coverage-title { align-items: flex-start; flex-direction: column; gap: 10px }.coverage-title>strong { font-size: 38px }.coverage-title p span { font-size: 11px }.coverage-legend { gap: 12px 18px }.sources-disclosure { margin: 0 -20px }.sources-disclosure>summary { padding: 14px 20px; font-size: 12px }
...
@media print {
  ...
  .finding,.inventory-group,.coverage-panel,.sources-disclosure,.table-scroll { overflow: visible }
  ...
  .finding { padding: 18px 20px 0; margin-bottom: 14px }.finding[hidden] { display: block!important }.finding h3 { font-size: 17px }.finding-topline { font-size: 9px; margin-bottom: 9px }.finding-clients { width: auto }.finding-summary,.next-step p,.impact p { font-size: 10px }.next-step { padding: 12px; margin: 12px 0 16px }.next-step h4,.impact h4,.affected h4 { font-size: 10px }.finding-evidence { margin: 0 -20px }.finding-evidence>summary,.sources-disclosure>summary { padding: 10px 20px; min-height: 30px; font-size: 10px }.evidence-count,.evidence-meta,.evidence-meta code,.reference-links,.section-note { font-size: 9px }.evidence-content { padding: 16px 20px }.reference-links a { min-height: 20px }
  ...
  table { font-size: 9px }thead th { font-size: 9px; padding: 8px }td,tbody th { padding: 8px }.evidence-table code,.evidence-table pre { font-size: 9px }.evidence-table,.sources-table { display: table }.evidence-table tbody,.sources-table tbody { display: table-row-group }.evidence-table thead,.sources-table thead { position: static; width: auto; height: auto; clip-path: none; display: table-header-group }.evidence-table tr,.sources-table tr { display: table-row }.evidence-table td,.sources-table td,.sources-table tbody th { display: table-cell }.evidence-table td:before,.sources-table td:before,.sources-table tbody th:before { display: none }
  .inventory-group[hidden] { display: block!important }.inventory-group>summary { min-height: 48px; padding: 12px 15px; break-after: avoid }.inventory-group-icon { width: 28px; height: 28px }.inventory-kind { display: flex }.inventory-kind strong { font-size: 11px; min-width: 180px }.inventory-kind>span,.inventory-count { font-size: 9px }.inventory-row[hidden] { display: table-row!important }.inventory-table { display: table }.inventory-table tbody { display: table-row-group }.inventory-table thead { position: static; width: auto; height: auto; display: table-header-group; clip-path: none }.inventory-table .inventory-row { display: table-row }.inventory-table tbody th,.inventory-table tbody td { display: table-cell; padding: 5px 8px; border-bottom: 1px solid var(--line) }.inventory-item-name strong,.inventory-item-name .observation-identity strong,.inventory-table .state { font-size: 9px }.inventory-path { white-space: normal; overflow-wrap: anywhere }.inventory-detail-label { display: none }.inventory-metadata>summary { min-height: 24px }.metadata-list { margin-top: 3px; padding: 5px 8px }.metadata-list dt,.metadata-list pre,.metadata-list code,.inventory-path,.inventory-context { font-size: 8px }
  .discovery-summary { grid-template-columns: repeat(4,minmax(0,1fr)); gap: 12px }.discovery-summary>div { padding: 12px }.discovery-summary dd { font-size: 20px }.discovery-summary dt { font-size: 9px }.coverage-panel { padding: 20px 20px 0 }.coverage-title { flex-direction: row; align-items: center }.coverage-title>strong { font-size: 32px }.coverage-title p { font-size: 11px }.coverage-title p span,.coverage-stat,.source-status,.source-reason,.inventory-location { font-size: 9px }.sources-disclosure { margin: 0 -20px }
