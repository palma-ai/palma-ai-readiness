#!/usr/bin/env python3
"""Build the standalone public skill from an explicit source allowlist."""
import argparse
import hashlib
import json
import re
...
def main():
    ...
    parser.add_argument("--source-commit", help="Full 40-character source commit SHA for release provenance")
    ...
    files = ["SKILL.md", "README.md", "LICENSE", "NOTICE", "SECURITY.md", "THIRD_PARTY_NOTICES.md", "agents/openai.yaml",
             ...
             *[f"scripts/palma_scan/{name}.py" for name in
               ("__init__", "cli", "model", "collector", "dedup", "rules", "report", "machine",
                ...
                "report_regulation", "report_overview", "report_brand", "marketplaces")],
             "scripts/palma_scan/palma_catalog.json", "scripts/palma_scan/marketplaces.json",
    ...
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*\.zip", args.output.name):
        parser.error("Use a simple .zip filename containing letters, digits, dots, underscores or hyphens.")
    ...
    build_info = {"formatVersion": 1, "sourceCommit": args.source_commit}
    contents.append(("BUILD-INFO.json", (json.dumps(build_info, sort_keys=True, indent=2) + "\n").encode("ascii")))
    contents.sort()
