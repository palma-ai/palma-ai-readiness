...
`artifacts-allowlisted-source` is Info for supported source metadata matching the maintained allowlist. `artifacts-unapproved-marketplace` is Critical for marketplace artifacts whose declared source is outside the allowlist; audit them before use. `artifacts-unresolved-source` is High for marketplace artifacts whose source record could not be resolved locally: their origin is unknown, not known to be unapproved, so the action is to verify the source. `skills-plugin-unreviewed` is High for packaged skills without marketplace metadata. These source rules preserve inventory and keep independent credential and access findings.
