...
Machine scope describes the intended collection surface, including inaccessible areas; it never means every byte or every client was inspected. Directory traversal reads names and metadata to find supported AI markers, not arbitrary project documents. General traversal excludes network/virtual mounts, dependency trees, caches, OS binary trees and application bundles. It also excludes other accounts' home folders (matched by path and by directory identity, so a firmlink or bind mount stays excluded), fixed OS temporary folders (`/tmp`, `/var/tmp`, macOS `/private/tmp`, `/private/var/tmp`, `/private/var/folders`, Windows `AppData/Local/Temp`) and the scanner's own folder; `TMPDIR` and similar variables are not consulted. Explicit `--workspace` paths are still collected. `scope.discovery.excludedDirectories` counts these skips. Known configuration and installation adapters inspect their selected sources separately. Inodes deduplicate overlapping local mount roots; symlinks/reparse points are not traversed. Defaults allow 500,000 directories, 5,000,000 entries and 30 minutes of project discovery, with a 100,000-entry per-directory limit. The Python API accepts larger explicit budgets. A reached limit creates a source gap and `scope.discovery.truncated`; it is never silently presented as exhaustive.
