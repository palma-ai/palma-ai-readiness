...
- **Critical:** potential credentials stored in configuration, explicit browser/computer capability and sensitive-action grants, and skills or plugins from unapproved or unresolved marketplaces. Audit unapproved marketplace components before use. The permission-only exceptions below remain Low.
- **High:** configured local MCP and direct remote MCP, concrete hooks, local skills and plugin skills without marketplace evidence, true sandbox-off settings, supported authentication concerns, side-loaded plugins, and oversized instruction components. Declarations establish available or configured capabilities, not execution or malicious behavior.
...
- **Info:** skills and plugins whose source metadata matches the maintained allowlist, plus disabled, cached, stale and managed-distribution context.

Keep observed state alongside every finding. A disabled or cached declaration may retain a
source or capability finding; it does not prove present execution. An unreadable source is
...
| `skills-local-unreviewed` | High | Local user or project skill without classified marketplace provenance; audit status remains separately stated. Repository indicators remain available as evidence |
...
`artifacts-allowlisted-source` is Info for supported source metadata matching the maintained allowlist. `artifacts-unapproved-marketplace` is Critical for marketplace artifacts with unapproved or unresolved source provenance; audit them before use. `skills-plugin-unreviewed` is High for packaged skills without marketplace metadata. These source rules preserve inventory and keep independent credential and access findings.
...
| Hooks and skills | Concrete configured hooks and local skills without marketplace evidence: High. Unapproved marketplace artifacts: Critical; allowlisted source evidence: Info. | Hook events may transmit prompts or tool inputs. Review destinations and handlers manually. Instructions and scripts are untrusted input. A file's existence is not activation or a prior audit result. [Claude hooks](https://code.claude.com/docs/en/hooks), [Claude skills](https://code.claude.com/docs/en/skills) |
...
and cannot establish repository provenance. Local skill review remains High; unresolved marketplace provenance remains Critical.
