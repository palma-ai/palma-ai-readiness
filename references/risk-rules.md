...
Source policy reviewed **14 September 2026**; priority policy version **2026-09-14.3**. The 29-rule [findings catalog](../scripts/palma_scan/palma_catalog.json) supplies detection conditions and original severity values. The explicit priority policy below determines displayed severity; `baselineSeverity` preserves the original value and `priorityPolicyVersion` identifies the applied policy. The report uses the Critical, High, Medium, Low and Info priorities below. Severity expresses Palma's governance priority for the observed condition, not a vendor vulnerability rating or proof of exploitation.
...
- **Critical:** potential credentials stored in configuration, explicit browser/computer capability and sensitive-action grants, and skills or plugins whose declared marketplace source is outside the allowlist. Audit unapproved marketplace components before use. The permission-only exceptions below remain Low.
- **High:** configured local MCP and direct remote MCP, concrete hooks, local skills and plugin skills without marketplace evidence, skills or plugins whose marketplace source could not be resolved locally, true sandbox-off settings, supported authentication concerns, side-loaded plugins, and oversized instruction components. Declarations establish available or configured capabilities, not execution or malicious behavior.
- **Medium:** unknown non-marketplace plugin origins, absent content digests and unknown transport, unless a specifically unsupported/malformed MCP shape is established.
- **Low:** permission bypass, automatic or absent approval prompts, unrestricted folder grants, duplicate skill content, and specifically malformed/unsupported MCP shapes. Group and highlight these observations instead of discarding them.
- **Info:** skills and plugins whose source metadata matches the maintained allowlist; disabled, cached, stale and managed-distribution context; and access, capability or permission findings none of whose declarations applies as written (each is in an unselected profile, in a cached policy copy, in a plugin pack that is switched off or cached without an installation record, or switched off).

Keep observed state alongside every finding. A disabled or cached declaration keeps its
source, capability or permission finding and its evidence; it does not prove present
execution. For the catalog rules in the access and permissions areas (local and direct
remote MCP, fixed-secret sign-in, computer and browser capabilities, permission bypass,
automatic approval, sandbox-off, hooks and broad agent tooling), the finding records how
many declarations **apply as written**: not in an unselected or shadowed profile, not in a
cached policy copy, not inside a plugin pack that is switched off or cached without an
installation record, and not switched off. The summary states the split, `applies` carries
the count, and a finding with no applying declaration is rated Info with a rating reason;
enabling or installing one restores the policy priority. Credential findings are unaffected:
a secret written in a disabled entry is still a secret in the file. A plugin pack counts as
installed through Claude's `installed_plugins.json` (matched by cache directory, or by its
marketplace, plugin and version folders for a copied home), Copilot's `installed-plugins`
folder, or Codex's remote installation record or `[plugins]` table; the `enabledPlugins` and
`[plugins]` switches give the pack's state. A Codex or Claude pack with none of these stays
"cached without an installation record". An unreadable source is
not a passed check. The absence of a local audit record does not establish that no audit has
occurred: retain **audit not assessed**. A configured connector need not have connected, and
an unverified gateway route does not establish the absence of other governance controls.

[Allowlisted source policy](trusted-marketplaces.md) documents exact origins, supported
installation metadata, and how maintainers curate the list. Allowlisting only changes
artifact-source priority. Credential, hook and access findings continue independently.
A marketplace name or cache folder alone never establishes an approved source. Ordinary
local skills without marketplace evidence receive High review priority rather than a
Critical finding inferred from installation alone.

## Documented sources

The default is **discovery for the signed-in account**: its profile and the machine's local volumes, locate project AI markers through directory metadata, then inspect known configuration and installation sources. Other accounts' home folders, OS temporary folders and the scanner's own folder are not traversed. Explicit `--copied-home` selects a separate offline directory inspection. The local collector supplies installation, package, editor-profile, custom-directory, settings, component, and state adapters; publishing and enrollment are removed. The source records in each report determine what that run actually inspected.
...
The evaluator retains catalog detection conditions and applies the versioned priority policy. Artifact-source findings supersede general origin and digest review for classified marketplace content. Findings carry the original `baselineSeverity`, the resulting severity, and `ratingReason`. Cached, disabled, stale, superseded and unselected-profile states remain attached to their evidence rather than silently removing a catalog match. Those states qualify application; they do not rewrite the stored declaration.

| Rule ID | Priority | Evidence condition |
| --- | --- | --- |
| `mcp-local-unaudited` | High | Local `stdio` or `sdk` MCP declaration |
| `mcp-inline-credential` | Critical | Potential credential stored inline in MCP configuration |
| `mcp-network-direct` | High | Direct or unverified network MCP route; connectors routed through a Palma-operated gateway host are governed and excluded |
| `mcp-static-secret-auth` | High | Static/bearer-header authentication instead of a short-lived sign-in; when every header holds a reference (an environment variable or client input) the summary says so. A declaration that stores the secret inline is reported once, under `mcp-inline-credential` |
...
| `skills-local-unreviewed` | High | Local user or project skill without classified marketplace provenance, shown as **Local skills without a recorded review**: the scan collected no review record, which is not evidence that no review happened. Repository indicators remain available as evidence |
...
| `hooks-declared` | High | Configured hook metadata |
...
`artifacts-allowlisted-source` is Info for supported source metadata matching the maintained allowlist. `artifacts-unapproved-marketplace` is Critical for installed marketplace artifacts whose declared source is outside the allowlist; audit them before use. `artifacts-unresolved-source` is High for installed marketplace artifacts whose source record could not be resolved locally: their origin is unknown, not known to be unapproved, so the action is to verify the source. A pack that is only downloaded, with no installation record and no enabled switch, stays under the Info `plugins-cached-only` finding whatever its source record says; an enabled switch counts as use, so the pack's connectors and provenance are rated as written even when no installation record could be read. `skills-plugin-unreviewed` is High for packaged skills without marketplace metadata. These source rules preserve inventory and keep independent credential and access findings.

Additional client-specific rules extend this coverage. Potential credentials found in any supported configuration receive Critical priority; concrete configured hooks receive High. `unrestricted-folder-access` groups broad folder-grant declarations at Low. `mcp-unsupported-shape` is Low when the parser specifically establishes a malformed or unsupported MCP shape. A general unknown transport keeps Medium.
...
Identical copies of a declaration in different files are merged during collection, so worktree checkouts, session copies and cached versions never multiply inventory counts or findings; a finding counts each merged declaration once, and its evidence lists the files that declare it. Copies merge only when their content is identical: different credential values, content, state or context stay separate, and a copy in a malformed file never merges with a well-formed one. Hooks are the one exception to the context rule: a hook whose handlers are identical merges across contexts, so the same notification command in a user file and a project file is one declaration to review, with both files listed. `skills-duplicate-content` is Low when skills share the same client, name and nonempty content digest in more than one remaining declaration, for example in both a user and a project skills folder that a client can load together. The finding includes all matching declarations. Missing digests, different clients, different names and different content are not merged. Copies may be intentional; review which are needed before consolidating them. Repetition alone does not establish that an agent loaded the copies or consumed additional tokens.
...
| Gemini MCP approvals | `mcpServers.<name>.trust = true`: Low automatic-approval review; the MCP declaration itself receives High priority. | `includeTools` restricts available tools; `excludeTools` wins on conflicts. Scope conclusions to that server. [MCP servers](https://geminicli.com/docs/tools/mcp-server/) |
...
| Network MCP | Direct or unverified remote MCP: High independently of credential evidence. | TLS protects data in transit; it does not establish governance. A client URL does not prove server bind address, authentication, reachability, or vulnerability. Loopback HTTP, `unix:`, and `pipe:` retain their actual transport classification. [MCP transports](https://modelcontextprotocol.io/specification/2025-11-25/basic/transports) |
| Local MCP and mutable packages | Configured stdio/SDK server: High. Mutable package versions add reproducibility evidence. | Local servers execute code with client privileges when launched unless isolated. Review provenance, pin a reviewed version, and minimize privileges. Do not launch commands to discover capabilities. [MCP security](https://modelcontextprotocol.io/docs/2025-11-25/tutorials/security/security_best_practices) |
| Missing remote scope evidence | Direct or unverified remote declarations keep High even when a local scope list is absent. | Say which controls are visible locally. OAuth, gateway controls, enterprise policy, and tool approval state may live elsewhere. Authorization is optional at protocol level; stdio uses a different model. [MCP authorization](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization) |
| Hooks and skills | Concrete configured hooks and local skills without marketplace evidence: High. Unapproved marketplace artifacts: Critical; unresolved marketplace provenance: High; allowlisted source evidence (including Codex's marker-backed bundled system skills): Info. | Hook events may transmit prompts or tool inputs. Review destinations and handlers manually. Instructions and scripts are untrusted input. A file's existence is not activation or a prior audit result. [Claude hooks](https://code.claude.com/docs/en/hooks), [Claude skills](https://code.claude.com/docs/en/skills) |
...
and cannot establish repository provenance. Local skill review remains High; unresolved marketplace provenance remains High, and a declared source outside the allowlist Critical.
